...
class ArrayToObject
{
    protected const string KEY_WIDTH = 'width';

    protected const string KEY_HEIGHT = 'height';

    protected const string KEY_ASPECT_RATIO = 'aspectRatio';
    ...
    protected function translate(string $key): void
    {
        ...
        if (($key === self::KEY_WIDTH && $this->has(self::KEY_HEIGHT) && $this->has(self::KEY_ASPECT_RATIO))) {
            $this->set(
                self::KEY_WIDTH,
                floor($this->getInt(self::KEY_HEIGHT) * $this->getFloat(self::KEY_ASPECT_RATIO))
            );
        }
    }
}
