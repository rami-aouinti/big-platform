...
use App\Calendar\Application\DataType\Coordinate;
use App\Calendar\Application\DataType\GPSPosition;
use Exception;
...
class GPSConverter
{
    final public const int ANGLE_360 = 360;

    final public const int ANGLE_180 = 180;

    final public const int ANGLE_90 = 90;

    final public const int ANGLE_0 = 0;

    final public const float ANGLE_360_0 = 360.;

    final public const float ANGLE_180_0 = 180.;

    final public const float ANGLE_157_5 = 157.5;

    final public const float ANGLE_112_5 = 112.5;

    final public const float ANGLE_90_0 = 90.;

    final public const float ANGLE_67_5 = 67.5;

    final public const float ANGLE_22_5 = 22.5;

    final public const float ANGLE_0_0 = 0.;

    final public const string REGEXP_ENCRYPTED_LATITUDE_LONGITUDE = '~!3d([0-9]+\.[0-9]+)+.+!4d([0-9]+\.[0-9]+)~';

    final public const string REGEXP_GOOGLE_REDIRECT = '~(https://maps.app.goo.gl/[a-zA-Z0-9]+)$~';

    final public const string REGEXP_GOOGLE_LOCATION_REDIRECT = '~^location: .+!3d([0-9]+\.[0-9]+)+.+!4d([0-9]+\.[0-9]+).+~m';

    final public const string REGEXP_SPLIT_LATITUDE_LONGITUDE = '~[, ]+~';

    final public const string REGEXP_DECIMAL = '[\-_]?\d+[.]\d+[°]*';

    final public const string REGEXP_DMS = '(?:[NEOWS]?)[ ]?(?:\d+°)[ ]?(?:\d+′)[ ]?\d+(?:(?:.\d+)?″)[ ]?(?:[NEOWS]?)';

    final public const string REGEXP_DMS_2 = '([NEOWS]?)[ ]?(\d+°)[ ]?(\d+′)[ ]?(\d+(?:.\d+)?″)[ ]?([NEOWS]?)';

    final public const array UNIT_DEGREE = [
        '°',
    ];

    final public const array UNIT_MINUTES = [
        '’',
        '\'',
        '′',
    ];

    final public const array UNIT_SECONDS = [
        '\'’',
        '"',
        '″',
    ];
    ...
    protected const int REGEXP_VERSION_1 = 1;

    protected const int REGEXP_VERSION_2 = 2;
