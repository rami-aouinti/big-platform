...
abstract class AbstractSpreadsheetRenderer
{
    ...
    public const string DURATION_FORMAT = '[hh]:mm';
    public const string DURATION_DECIMAL = '#0.00';
    ...
    public const string RATE_FORMAT_DEFAULT = '#.##0,00 [$%1$s];-#.##0,00 [$%1$s]';
    public const string RATE_FORMAT_LEFT = '_("%1$s"* #,##0.00_);_("%1$s"* -#,##0.00;_("%1$s"* "-"??_);_(@_)';
    public const string RATE_FORMAT_RIGHT = '_(* "%1$s" #,##0.00_);_(* "%1$s" -#,##0.00;_(* "%1$s" "-"??_);_(@_)';

    /**
     * @internal used in html to excel exporter
     */
    public const string RATE_FORMAT_NO_CURRENCY = '#,##0.00;-#,##0.00';
    ...
    protected function setFormattedTime(Worksheet $sheet, int $column, int $row, ?DateTime $date): void
    {
        ...
        $sheet->setCellValue(CellAddress::fromColumnAndRow($column, $row), $excelDate);
        $sheet->getStyle(
            CellAddress::fromColumnAndRow($column, $row)
    ...
    protected function setDurationTotal(Worksheet $sheet, int $column, int $row, string $startCoordinate, string $endCoordinate): void
    {
        $sheet->setCellValue(
            CellAddress::fromColumnAndRow($column, $row),
            sprintf('=SUBTOTAL(9,%s:%s)', $startCoordinate, $endCoordinate)
        );
    ...
    protected function setDuration(Worksheet $sheet, int $column, int $row, ?int $duration): void
    {
        ...
        $sheet->setCellValue(
            CellAddress::fromColumnAndRow($column, $row),
            sprintf('=%s/%s', $duration, $this->durationBase)
        );
        $sheet->getStyle(
            CellAddress::fromColumnAndRow($column, $row)
        )->getNumberFormat()->setFormatCode(
    ...
    protected function setRateTotal(
        Worksheet $sheet,
        int $column,
        int $row,
    ...
    protected function getColumns(array $exportItems, TimesheetQuery $query, array $columns): array
    {
        ...
        if ($showRates && isset($columns['rate_internal']) && !isset($columns['rate_internal']['render'])) {
            $columns['rate_internal']['render'] = function (
                Worksheet $sheet,
                int $row,
                int $column,
                ExportableItem $entity
            ) {
                $currency = '';
                if ($entity->getProject() !== null) {
                    $currency = $entity->getProject()->getCustomer()->getCurrency();
        ...
        if (isset($columns['username'])) {
            if (!isset($columns['username']['render'])) {
                $columns['username']['render'] =
                    function (Worksheet $sheet, int $row, int $column, ExportableItem $entity) {
                        $username = '';
                        if ($entity->getUser() !== null) {
        ...
        if (isset($columns['description']) && !isset($columns['description']['render'])) {
            $maxWidth = array_key_exists(
                'maxWidth',
                $columns['description']
            ) &&
            is_numeric($columns['description']['maxWidth']) ? (int)$columns['description']['maxWidth'] : null;
            $wrapText = array_key_exists(
                'wrapText',
                $columns['description']
            ) && (bool)$columns['description']['wrapText'];
            $sanitizeText = !array_key_exists(
                'sanitizeDDE',
                $columns['description']
            ) || (bool)$columns['description']['sanitizeDDE'];
        ...
        if (isset($columns['timesheet-meta'])) {
            $timesheetMetaFields = $this->findMetaColumns(
                new TimesheetMetaDisplayEvent($query, TimesheetMetaDisplayEvent::EXPORT)
            );
            ...
            $columns['timesheet-meta'] = [
                'header' => function (Worksheet $sheet, int $row, int $column) use ($timesheetMetaFields): int {
                    foreach ($timesheetMetaFields as $metaField) {
                        $sheet->setCellValue(
                            CellAddress::fromColumnAndRow($column++, $row),
                            $this->translator->trans($metaField->getLabel())
                        );
                    }
        ...
        if (isset($columns['customer-meta'])) {
            ...
            $customerMetaFields = $this->findMetaColumns(
                new CustomerMetaDisplayEvent($customerQuery, CustomerMetaDisplayEvent::EXPORT)
            );
        ...
        if (isset($columns['project-meta'])) {
            $projectMetaFields = $this->findMetaColumns(
        ...
        if (isset($columns['activity-meta'])) {
            $activityMetaFields = $this->findMetaColumns(
