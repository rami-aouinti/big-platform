    ...
    public function wizard(Request $request, UserService $userService, string $wizard): Response
    {
        ...
        if ($wizard === 'profile') {
            ...
            if ($form->isSubmitted() && $form->isValid()) {
                ...
                if ($data['reload'] === '1') {
                    return $this->redirectToRoute('wizard', ['wizard' => 'profile', '_locale' => $user->getLanguage()->value]);
                } else {
                    return $this->redirectToRoute('wizard', ['wizard' => $next, '_locale' => $user->getLanguage()->value]);
                }
            }
